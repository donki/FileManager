...
{
    private const string AppcastUrl = "https://raw.githubusercontent.com/donki/FileManager/main/appcast.json";
    ...
    public UpdateService(ILocalizationService localization)
    {
        _l = localization;
    }
    ...
    public async Task CheckAndPromptAsync(Page page)
    {
        ...
        try
        {
            ...
            var wantsUpdate = await page.DisplayAlert(
                _l["UpdateAvailableTitle"],
                string.Format(_l.CurrentCulture, _l["UpdateAvailableMessage"], manifest.Version, current),
